...
function Badge({ ok, label }: { ok: boolean; label: string }) {
  return (
    <span
      className={`${badgeStyles} ${ok ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-amber-300 bg-amber-50 text-amber-800"}`}
    >
    ...
    <div aria-live="polite" className="flex flex-col items-center gap-4">
      ...
      {status.kind === "error" && (
        <div className="flex flex-col items-center gap-3">
          <p className="text-sm text-red-700">Could not reach the API: {status.message}</p>
          <button
            type="button"
            onClick={retry}
