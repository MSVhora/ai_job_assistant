...
**Status:** Implemented (branch `v4/36-duplicate-concurrent-run-guard`; implementation notes: the plan's hard-fail-on-unbackfilled-rows was wrong — the dev DB holds pre-one-source-era rows with `query.sources: null`, so migration 0017 backfills from `query->>'source'` else `query->'sources'->>0`, then deletes rows with neither (destructive for those rows only — terminal, pre-source-field, useless history); non-terminal statuses are `pending`/`running` as this note predicted from the enum; `client.ts` error handling was restructured (`ApiError` now carries the raw response `body`, `parseErrorBody`/`fail` helpers) so `startJobSearch` can rethrow 409s as typed `DuplicateRunError` without a second body read; route-level tests pre-seed an active run because TestClient drains BackgroundTasks before the response returns. `TEST_DATABASE_URL=postgresql+asyncpg://postgres:postgres@localhost:5432/ai_job_assistant_test` runs the full suite green (417 tests))
