...
| Contract/type booleans | `AdzunaJobSource.filters()` declares four new optional boolean filters: `full_time`, `part_time`, `contract`, `permanent` (mapped to the like-named Adzuna params when `True`). **No auto-derivation** — no profile/request field carries a job-type preference today (owner decision, this issue): heuristics need a schema-level signal, which is Problem 7 / salary-band territory | Scope item 3 honored via the existing declared-options mechanism: power users get the knobs, zero new required fields, zero UI work (the filters form renders from `SourceInfoResponse.filters` automatically) |
