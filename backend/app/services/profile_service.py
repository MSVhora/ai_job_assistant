...
async def _hash_for_current_sources(
    session: AsyncSession, structured: StructuredProfile
) -> str | None:
    """Compute the generation-input hash against the enabled sources.

    None when no sources are enabled yet — stored specs get regenerated by the
    next save/chat completion regardless, since no prior hash can match.
    """
    try:
        ...
        return query_builder.compute_queries_input_hash(
            structured, [source.name for source in enabled], declaration_map
        )
...
def schedule_query_refresh(background_tasks: BackgroundTasks, profile_id: uuid.UUID) -> None:
    background_tasks.add_task(_refresh_queries_background, profile_id)
    logger.info("queries.refresh_scheduled profile_id=%s", profile_id)
...
def _profile_response(
    ...
    from app.services.gap_fill import missing_fields as gap_fill_missing_fields
    ...
    return ProfileResponse(
        ...
        missing_fields=[field.key for field in gap_fill_missing_fields(structured)],
    )
